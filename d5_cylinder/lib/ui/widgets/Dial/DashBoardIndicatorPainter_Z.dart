...
class DashBoardIndicatorPainter_Z extends CustomPainter {
  final double speeds;
  double tableSpace;
...
//  final Picture pictureBackView;
  ...
  DashBoardIndicatorPainter_Z(this.speeds, this.tableSpace,
      this.pictureBackGround, this.pictureIndicator);

  @override
  void paint(Canvas canvas, Size size) {
    String text = speeds.toStringAsFixed(1);
    pointerValue =  conversionCoordZPosition(speeds);
...
//    textPainter.layout(minWidth: 60,maxWidth: 40);
  ...
  void drawIndicator(Canvas canvas, Size size) {
    ...
    canvas.save();
    canvas.translate(halfWidth, halfHeight);
    canvas.rotate((-(tableCount_z * 0.75)/2.0 + pointerValue) * tableSpace);
    canvas.translate(-halfWidth, -halfHeight);
    canvas.drawPicture(pictureIndicator);
    canvas.restore();
  }

  double conversionCoordZPosition(double value) {
    double z = 0;
    if (value == 0) {
      z = 33;
    } else if (value > 0) {
